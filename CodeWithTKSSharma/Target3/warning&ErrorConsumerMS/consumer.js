...
class Consumer{
    ...
    async consumeMessages(){
        if(!this.channel) await this.createChannel();
            ...
            await this.channel.bindQueue(q.queue,config.rabbitMQ.exchangeName,config.rabbitMQ.routingKey1);
            await this.channel.bindQueue(q.queue,config.rabbitMQ.exchangeName,config.rabbitMQ.routingKey2);
            // routing key= binding key proucer routign key bheja binding key se match
            // here Warning and Error se same queue binding key
            
            // one queue to multiple binding
